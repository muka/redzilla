...
lib.handler = function (req, res, next) {
    ...
    if(!isAdminPath && !instance) {
        logger.debug("Not an admin path nor instance detected")
        return authorized()
    }

    var user = basicAuth(req)
    if(!user || !user.name || !user.pass) {
        logger.debug("Account data not provided")
        return unauthorized()
    }

    if(isAdminPath) {
        logger.debug("Admin pages authentication of " + user.name)
        return lib.authRoot(user) ? authorized() : unauthorized()
    }
    else if(instance) {
        logger.debug("Authentication of " + user.name + " on instance " + instance.name)
