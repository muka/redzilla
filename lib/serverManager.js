...
lib.setup = function () {
  return new Promise(function (ok, ko) {
    ...
    logger.debug("Setup main http server")
    ...
    if(standalone) {
      logger.debug("Creating main server")
        // add access log
 ...
 * @param {boolean} force proxy reinit
 * @return {Promise}
 */
  ...
  return new Promise(function (ok, ko) {
    ...
    if(force) {
      ...
      if(proxies[name]) {
        ...
        try {
          ...
          logger.warn("Error closing proxy: %s", e.message)
          logger.error(e)
        }
        proxies[name] = null
      } else {
        logger.silly("Proxy not found")
      }
    }
    ...
    if(!proxies[name]) {
      ...
      if(!instance) {
        ...
        return ko(new Error("Instance not available"))
      }
      ...
      proxy.on('open', function (proxySocket) {
        logger.silly("Opened new proxy for %s at %s", name, url)
        proxies[name] = proxy
        ok(proxy)
      })
      ...
      proxy.redz = { retry: 0, timer: null }
      proxy.on('error', function (err, req, res) {

        logger.warn("Proxy error for %s: %s", name, err.message)
        logger.error(err)
        ...
        if(proxy.redz.retry < 4) {
          proxy.redz.timer = setTimeout(function () {
            logger.info("Restarting proxy %s at %s", name, url)
            lib.getProxy(name, true)
              .then(function (e) {
                proxy.redz.retry = -1
                logger.silly("Restarted correctly")
              })
              .catch(function (e) {
                logger.warn("Exception restarting: %s", e.message)
              })
              .finally(function() {
                proxy.redz.retry++;
                clearTimeout(proxy.redz.timer)
                proxy.redz.timer = null
              })
          }, 2500)
        }
        else {
    ...
    logger.silly("Proxy for %s exists", name)
    ok(proxies[name])
  })
}
...
lib.createInstanceProxy = function (instance) {
  ...
  logger.silly("Proxying %s/* to %s", userPath, localUri)
  ...
  app.all(regUrlMatch, function (preq, pres, next) {
    ...
    var _proxyRequest = function () {
      ...
      lib.getProxy(instance.name)
        .then(function(proxy) {
          proxy.web(preq, pres, {
            target: require('url').parse(localUri)
          }, function (e, req, res) {
...
lib.setupProxy = function (app) {
  ...
  logger.debug("Set websocket proxy")
  app.server.on('upgrade', function (preq, socket, head) {
    ...
    if(instance) {
      ...
      lib.getProxy(instance.name)
        ...
        .catch(function(e) {
          logger.warn("WS Proxy not available: %s", e.message)
        })
  ...
  logger.debug("Set http proxy %s", userPathPrefix + '/:name')
  app.all(userPathPrefix + '/:name', function (req, res, next) {

    logger.silly("Received proxy request for %s %s", req.method, req.url)
    ...
    if(!instance) {
      ...
      logger.silly("Instance not available")
      ...
      if(config.get('createOnRequest', false)) {
        ...
        logger.silly("Create on request disabled, responding not found")
        return res.sendStatus(404)
      }
    } else {
      logger.silly("Found instance to proxy")
    }
    ...
    var checkProxy = function () {
      ...
      if(!lib.hasProxy(name)) {
        ...
        lib.getProxy(name)
          ...
          .catch(function(err) {
            logger.warn("Cannot start proxy: %s", err.message)
          })

        return
      }

      logger.silly("Proxy exists for instance %s", name)
      return next()
    }
    ...
    processManager.start(instance)
      ...
      .catch(function (e) {
        logger.warn("Cannot start instance for %s: %s", name, e.message)
        logger.error(e)
      })
        ...
        switch(op) {
          ...
          case "restart":
            return instance.status().then(function (response) {
                ...
                if(op === "start" && (response.status === util.state.started || response.status === util.state.restarting)) {
                  logger.silly("Instance %s is running, skip start", name)
                  return Promise.resolve(response)
                }

                if(op === "stop" && response.status === util.state.stopped) {
