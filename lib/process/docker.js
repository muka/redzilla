...
lib.start = function (config) {
    return new Promise(function (ok, ko) {
        ...
        try {
            ...
            volumes[ getConfig().volumes.userDir ] = {};
            volumes[ getConfig().volumes.nodesDir ] = {};
            
            var userDir = path.resolve(_config.getInstancesDir() + "/" + config.uid);
            
            var mountPoints = {
                userDir: userDir,
                nodesDir: userDir, // @TODO review this!
            };
...
lib.status = function (name) {
    return new Promise(function(ok, ko) {
        ...
        lib.instance(name).inspect(function(err, res) {
...
lib.kill = function (name) {
    return Promise(function() {
        ...
        if (processes[name]) {
            ...
            processes[name].kill(function(err, res) {
...
lib.stop = function() {
    ...
    return Promise.all(_k).each(lib.kill);
};
...
process.on('SIGINT', function () {
    ...
    lib.stop().then(_kill).catch(_kill);
});