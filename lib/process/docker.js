  ...
  return new Promise(function (ok, ko) {
    ...
    getDocker().pull(imageName, function (err, stream) {
      ...
      stream.on("data", function (m) {
        logger.silly("Docekr pull: %s", JSON.parse(m.toString()).status)
      })
      stream.once('end', () => {
        logger.error("Fetched image %s", imageName)
        ok()
      })
    })
  })
    ...
    .then(function () {
      return new Promise(function (ok, ko) {
        ...
        try {
          ...
          logger.debug("Mount points", mountPoints)
          ...
          let containerName = "redzilla_" + name

          let createContainer = (ok1, ko1) => {
              ...
              container.inspect(function (err, data) {
                if(err) {
                  if(err.statusCode === 404) {
                    return createContainer(ok1, ko1)
                  }
                  return ko1(err)
                }
          ...
          let startContainer = (container) => {
            ...
            container.attach({
              ...
              processLogger.debug("Volume binding %j", _binds)
              ...
              console.warn("docker start options: ", startOptions)
