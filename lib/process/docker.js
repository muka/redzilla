    ...
    .then(function () {
      return new Promise(function (ok, ko) {
        ...
        try {
          ...
          logger.debug("Mount points: %j", mountPoints)
          ...
          logger.debug("Starting `git %s`",  name)
          processLogger.debug("Creating container, image %s", getConfig().image)
          ...
          docker.createContainer({
            ...
            container.attach({
              ...
              if(err) {
                logger.error("Error attaching to container: %j", err.message)
                tryRelaunch(err)
                return ko(err)
              }
