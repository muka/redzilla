...
var instancesDir = function() {
    return config.instancesDir || path.resolve(__dirname + "/../../instances");
}

var instancesCacheFile = function() {
    return instancesDir() + "/cache.json";
};
...
lib.setup = function (dir, settings) {
    ...
    red_linkables.forEach(function (filename) {
        ...
        var src = path.resolve(red_path() + "/" + filename + "/");
    ...
    // copy module only
    ncp(path.resolve(red_path() + "/red.js"), path.resolve(dir + "/red.js"));
...
lib.getPort = function () {
    ...
    return lib.getUsedPort().then(function (ports) {
        ...
        config.basePort++;
        while (ports[ config.basePort ]) {
            config.basePort++;
            if (config.basePort > 65000) {
                return Promise.reject(new Error("Cannot allocate port " + config.basePort));
            }
        }

        logger.debug("Selected port " + config.basePort);
        return Promise.resolve(config.basePort);
    });
};
