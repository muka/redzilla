...
lib.prepareConfig = function(name, iconf) {
    return util.prepareConfig(name, iconf).then(function(conf) {
        ...
        conf.userDir  = path.resolve(__dirname + '/../../' + getConfig().userDir)
        conf.nodesDir = path.resolve(__dirname + '/../../' + getConfig().nodesDir)
...
lib.stop = function(name, opts) {
    ...
    if(_k.length === 0) return Promise.resolve()
